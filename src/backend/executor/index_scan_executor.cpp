...
IndexScanExecutor::IndexScanExecutor(const planner::AbstractPlan *node,
    ...
    : AbstractScanExecutor(node, executor_context),
      is_blind_write_(is_blind_write) {}
...
bool IndexScanExecutor::ExecPrimaryIndexLookup() {
  ...
  for (auto tuple_location_ptr : tuple_location_ptrs) {
    ...
    cid_t max_committed_cid = (gc::GCManagerFactory::GetGCType() == GC_TYPE_CO)
                                  ? transaction_manager.GetMaxCommittedCid()
                                  : 0;
    while (true) {
      ++chain_length;

      auto visibility = transaction_manager.IsVisible(tile_group_header,
      ...
      // if the tuple is visible.
        ...
        if (predicate_ == nullptr) {
          ...
          if (is_blind_write_ == false &&
              concurrency::current_txn->IsStaticReadOnlyTxn() == false) {
      ...
      // if the tuple is not visible.
        ...
        if (concurrency::TransactionManagerFactory::GetProtocol() ==
            ...
            tile_group_header->GetTransactionId(tuple_location.offset) ==
                INITIAL_TXN_ID &&
            tile_group_header->GetEndCommitId(tuple_location.offset) <=
                concurrency::current_txn->GetBeginCommitId()) {
          // See an invisible version that does not belong to any one in a new
          // to old version chain.
          // In such case, we assert that there should be either a deleted
          // version or a newly updated version.
          // So we just wire back using the index head ptr stored in the reserve
          // field.
          tuple_location =
              *((concurrency::OptimisticN2OTxnManager *)(&transaction_manager))
        ...
        if (concurrency::TransactionManagerFactory::GetProtocol() ==
          ...
          // Wire back
          tuple_location =
              *((concurrency::TsOrderN2OTxnManager *)(&transaction_manager))
                   ->GetHeadPtr(tile_group_header, tuple_location.offset);
          tile_group = manager.GetTileGroup(tuple_location.block);
          tile_group_header = tile_group.get()->GetHeader();
          chain_length = 0;
        ...
        if (tuple_location.IsNull()) {
          ...
          // For speculative read, a transaction may incidentally miss a visible
          // tuple due to a non-atomic
          // timestamp update. In such case, we just return false and abort the
          // txn.
          transaction_manager.SetTransactionResult(RESULT_FAILURE);
          transaction_manager.AddOneReadAbort();
          return false;
        ...
        assert(concurrency::TransactionManagerFactory::GetProtocol() !=
                   CONCURRENCY_TYPE_TO_N2O &&
        ...
        if (old_end_cid <= max_committed_cid) {
          ...
          if (tile_group_header->SetAtomicTransactionId(
            ...
            // its ok to use any tile group, as it will not change the table
            // pointed to.
            storage::DataTable *table = dynamic_cast<storage::DataTable *>(
            ...
            if (index_count != 1) {
              ...
              for (size_t idx = 0; idx < table->GetIndexCount(); ++idx) {
                ...
                if (index->GetIndexType() !=
                  ...
                  key->SetFromTuple(expired_tuple.get(), indexed_columns,
                                    index->GetPool());
            ...
            // Continue the while loop with the new header we get the index head
            // ptr
            continue;
          }
        }
...
// the RB chain.
void IndexScanExecutor::RBVerifyVisible(
    std::vector<ItemPointer> &tuple_locations) {
  auto &indexed_columns =
      index_->GetMetadata()->GetKeySchema()->GetIndexedColumns();
  auto &manager = catalog::Manager::GetInstance();
  ...
  for (auto itr = tuple_locations.begin(); itr != tuple_locations.end();) {
    ...
    for (oid_t indexed_column : indexed_columns) {
      ...
      key_tuple.SetValue(id_map[indexed_column], value,
                         executor_context_->GetExecutorContextPool());
    }
...
bool IndexScanExecutor::ExecSecondaryIndexLookup() {
  ...
  if (0 == key_column_ids_.size()) {
    ...
    if (concurrency::TransactionManagerFactory::IsRB()) {
      if (index::IndexFactory::GetSecondaryIndexType() ==
          SECONDARY_INDEX_TYPE_VERSION) {
        index_->Scan(values_, key_column_ids_, expr_types_,
                     SCAN_DIRECTION_TYPE_FORWARD, rb_tuple_locations);
        for (auto &rb_item_ptr : rb_tuple_locations) {
          tuple_locations.push_back(rb_item_ptr.location);
        }
      } else {
  ...
  for (auto tuple_location : tuple_locations) {
    ...
    // if the tuple is visible.
    auto visibility =
        transaction_manager.IsVisible(tile_group_header, tuple_id);
...
bool IndexScanExecutor::ExecTupleSecondaryIndexLookup() {
  ...
  PL_ASSERT(index::IndexFactory::GetSecondaryIndexType() ==
            SECONDARY_INDEX_TYPE_TUPLE);
