...
void CreateWarehouseTable() {
  ...
  auto w_id_column = catalog::Column(
      VALUE_TYPE_INTEGER, GetTypeSize(VALUE_TYPE_INTEGER), "W_ID", is_inlined);
...
void CreateCustomerTable() {
  ...
  if (GetSKeyIndexType() == INDEX_TYPE_HASH) {
    ...
    skey_index = index::IndexFactory::GetInstance(
        index_metadata, state.warehouse_count * state.districts_per_warehouse *
                            state.customers_per_district);
  } else {
...
void CreateNewOrderTable() {
  ...
  if (state.index == INDEX_TYPE_HASH) {
    pkey_index = index::IndexFactory::GetInstance(
        index_metadata, state.warehouse_count * state.districts_per_warehouse *
                            state.new_orders_per_district * preallocate_scale);
  } else {
    pkey_index = index::IndexFactory::GetInstance(index_metadata);
  }
...
void CreateOrderLineTable() {
  ...
  if (state.index == INDEX_TYPE_HASH) {
    pkey_index = index::IndexFactory::GetInstance(
        index_metadata, state.warehouse_count * state.districts_per_warehouse *
                            state.customers_per_district * orders_max_ol_cnt *
                            preallocate_scale);
  } else {
    pkey_index = index::IndexFactory::GetInstance(index_metadata);
  }
...
void GetStringFromValue(const Value &value, std::string &str) {
  ...
  if (value.m_sourceInlined) {
    str_bytes = *reinterpret_cast<char *const *>(value.m_data) +
                value.GetObjectLengthLength();
  } else {
...
std::unique_ptr<storage::Tuple> BuildCustomerTuple(
  ...
  // C_PAYMENT_CNT
  customer_tuple->SetValue(
      18, ValueFactory::GetIntegerValue(customers_init_payment_cnt), nullptr);
  // C_DELIVERY_CNT
  customer_tuple->SetValue(
      19, ValueFactory::GetIntegerValue(customers_init_delivery_cnt), nullptr);
  // C_DATA
...
void LoadTPCCDatabase() {
  ...
  if (state.warehouse_count < loading_thread_count) {
    ...
    for (int thread_id = 0; thread_id < state.warehouse_count; ++thread_id) {
      ...
      load_threads[thread_id]
          .reset(new std::thread(LoadWarehouses, warehouse_from, warehouse_to));
    }
