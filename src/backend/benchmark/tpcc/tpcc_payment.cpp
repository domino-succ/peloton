...
Payment *GeneratePayment() {
  ...
  payment->customer_pindex_scan_executor_ = customer_pindex_scan_executor;
  payment->customer_index_scan_executor_ = customer_index_scan_executor;
  payment->customer_update_bc_index_scan_executor_ =
      customer_update_bc_index_scan_executor;
  payment->customer_update_bc_executor_ = customer_update_bc_executor;
  payment->customer_update_gc_index_scan_executor_ =
      customer_update_gc_index_scan_executor;
  payment->customer_update_gc_executor_ = customer_update_gc_executor;

  payment->warehouse_index_scan_executor_ = warehouse_index_scan_executor;
  payment->warehouse_update_index_scan_executor_ =
      warehouse_update_index_scan_executor;
  payment->warehouse_update_executor_ = warehouse_update_executor;

  payment->district_index_scan_executor_ = district_index_scan_executor;
  payment->district_update_index_scan_executor_ =
      district_update_index_scan_executor;
  payment->district_update_executor_ = district_update_executor;
...
void SetPayment(Payment *payment) {
  ...
  // PREPARE ARGUMENTS
  /////////////////////////////////////////////////////////
  payment->warehouse_id_ = GenerateWarehouseId();
  payment->district_id_ =
      GetRandomInteger(0, state.districts_per_warehouse - 1);
  payment->customer_id_ = GetRandomInteger(0, state.customers_per_district - 1);
...
bool Payment::Run() {
  ...
  */
  ...
  // Execute the query
  auto warehouse_list = ExecuteReadTest(warehouse_index_scan_executor_);
  ...
  // Execute the query
  auto district_list = ExecuteReadTest(district_index_scan_executor_);
  ...
  if (customer_credit == customers_bad_credit) {
    ...
    customer_update_bc_index_scan_executor_->ResetState();
    ...
    customer_update_bc_index_scan_executor_->SetValues(customer_pkey_values);
    ...
    customer_update_bc_executor_->SetTargetList(customer_target_list);

    // Execute the query
    ExecuteUpdateTest(customer_update_bc_executor_);
  } else {
    LOG_TRACE(
    ...
    customer_update_gc_index_scan_executor_->ResetState();
    ...
    customer_update_gc_index_scan_executor_->SetValues(customer_pkey_values);
    ...
    customer_update_gc_executor_->SetTargetList(customer_target_list);

    // Execute the query
    ExecuteUpdateTest(customer_update_gc_executor_);
  }
...
bool RunPayment(Payment *payment) {
  /*
     "PAYMENT": {
     "getWarehouse": "SELECT W_NAME, W_STREET_1, W_STREET_2, W_CITY, W_STATE,
     W_ZIP FROM WAREHOUSE WHERE W_ID = ?", # w_id
  ...
  // PREPARE ARGUMENTS
  /////////////////////////////////////////////////////////
  int warehouse_id = payment->warehouse_id_;
  int district_id = payment->district_id_;
  int customer_warehouse_id = payment->customer_warehouse_id_;
  int customer_district_id = payment->customer_district_id_;
  int customer_id = payment->customer_id_;
  std::string customer_lastname = payment->customer_lastname_;
  double h_amount = payment->h_amount_;
  ...
  payment->SetContext(context.get());
  ...
  if (customer_id >= 0) {
    ...
    payment->customer_pindex_scan_executor_->ResetState();
    ...
    payment->customer_pindex_scan_executor_->SetValues(customer_pkey_values);

    auto customer_list =
        ExecuteReadTest(payment->customer_pindex_scan_executor_);
    ...
    payment->customer_index_scan_executor_->ResetState();
    ...
    payment->customer_index_scan_executor_->SetValues(customer_key_values);

    auto customer_list =
        ExecuteReadTest(payment->customer_index_scan_executor_);
  ...
  payment->warehouse_index_scan_executor_->ResetState();

  std::vector<Value> warehouse_key_values;

  warehouse_key_values.push_back(ValueFactory::GetIntegerValue(warehouse_id));

  payment->warehouse_index_scan_executor_->SetValues(warehouse_key_values);

  // Execute the query
  auto warehouse_list =
      ExecuteReadTest(payment->warehouse_index_scan_executor_);
  ...
  payment->district_index_scan_executor_->ResetState();
  ...
  payment->district_index_scan_executor_->SetValues(district_key_values);

  // Execute the query
  auto district_list = ExecuteReadTest(payment->district_index_scan_executor_);
  ...
  payment->warehouse_update_index_scan_executor_->ResetState();

  payment->warehouse_update_index_scan_executor_->SetValues(
  ...
  payment->warehouse_update_executor_->SetTargetList(warehouse_target_list);

  // Execute the query
  ExecuteUpdateTest(payment->warehouse_update_executor_);
  ...
  payment->district_update_index_scan_executor_->ResetState();

  payment->district_update_index_scan_executor_->SetValues(district_key_values);
  ...
  payment->district_update_executor_->SetTargetList(district_target_list);

  // Execute the query
  ExecuteUpdateTest(payment->district_update_executor_);
  ...
  if (customer_credit == customers_bad_credit) {
    ...
    payment->customer_update_bc_index_scan_executor_->ResetState();
    ...
    payment->customer_update_bc_index_scan_executor_->SetValues(
    ...
    payment->customer_update_bc_executor_->SetTargetList(customer_target_list);

    // Execute the query
    ExecuteUpdateTest(payment->customer_update_bc_executor_);
  } else {
    LOG_TRACE(
        "updateGCCustomer: # c_balance = %f, c_ytd_payment = %f, c_payment_cnt "
        "= %d, c_w_id = %d, c_d_id = %d, c_id = %d",
    ...
    payment->customer_update_gc_index_scan_executor_->ResetState();
    ...
    payment->customer_update_gc_index_scan_executor_->SetValues(
        customer_pkey_values);
    ...
    payment->customer_update_gc_executor_->SetTargetList(customer_target_list);

    // Execute the query
    ExecuteUpdateTest(payment->customer_update_gc_executor_);
  }
