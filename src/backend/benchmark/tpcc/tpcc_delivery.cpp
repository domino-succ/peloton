...
Delivery *GenerateDelivery() {
  ...
  new_order_expr_types.push_back(ExpressionType::EXPRESSION_TYPE_COMPARE_EQUAL);
  new_order_expr_types.push_back(
  ...
  planner::IndexScanPlan new_order_idex_scan_node(
      new_order_table, nullptr, new_order_column_ids, new_order_idex_scan_desc);
  ...
  new_order_delete_expr_types.push_back(
  ...
  executor::IndexScanExecutor *new_order_delete_index_scan_executor =
      new executor::IndexScanExecutor(&new_order_delete_idex_scan_node,
                                      nullptr);
  ...
  order_line_expr_types.push_back(
      ExpressionType::EXPRESSION_TYPE_COMPARE_EQUAL);
  order_line_expr_types.push_back(
      ExpressionType::EXPRESSION_TYPE_COMPARE_EQUAL);
  order_line_expr_types.push_back(
  ...
  executor::IndexScanExecutor *order_line_update_index_scan_executor =
      new executor::IndexScanExecutor(&order_line_update_index_scan_node,
                                      nullptr);
  ...
  delivery->new_order_index_scan_executor_ = new_order_index_scan_executor;
  delivery->new_order_delete_index_scan_executor_ =
      new_order_delete_index_scan_executor;
  delivery->new_order_delete_executor_ = new_order_delete_executor;
  delivery->orders_index_scan_executor_ = orders_index_scan_executor;
  delivery->orders_update_index_scan_executor_ =
      orders_update_index_scan_executor;
  delivery->orders_update_executor_ = orders_update_executor;
  delivery->order_line_index_scan_executor_ = order_line_index_scan_executor;
  delivery->order_line_update_index_scan_executor_ =
      order_line_update_index_scan_executor;
  delivery->order_line_update_executor_ = order_line_update_executor;
  delivery->customer_index_scan_executor_ = customer_index_scan_executor;
  delivery->customer_update_executor_ = customer_update_executor;

  // Set values
  // SetPayment(payment);

  //
  delivery->SetRegionCover();
...
void SetDelivery(Delivery *delivery) {
  ...
  // PREPARE ARGUMENTS
  /////////////////////////////////////////////////////////
  delivery->warehouse_id_ = GenerateWarehouseId();
  delivery->o_carrier_id_ =
  ...
  delivery->primary_keys_.assign(1, delivery->warehouse_id_);
}
...
bool Delivery::Run() {
  ...
  int o_carrier_id = o_carrier_id_;
  // int64_t ol_delivery_d = time(0);
  ...
  for (int d_id = 0; d_id < state.districts_per_warehouse; ++d_id) {
    ...
    new_order_index_scan_executor_->ResetState();
    ...
    new_order_index_scan_executor_->SetValues(new_order_key_values);

    // WORKAROUND: let's hard-code the limit executor for now (due to caching
    // reason). --Yingjun
    ...
    auto new_order_ids = ExecuteReadTest(new_order_index_scan_executor_);
    ...
    orders_index_scan_executor_->ResetState();
    ...
    orders_index_scan_executor_->SetValues(orders_key_values);

    auto orders_ids = ExecuteReadTest(orders_index_scan_executor_);
    ...
    order_line_index_scan_executor_->ResetState();
    ...
    order_line_key_values.push_back(
        ValueFactory::GetIntegerValue(warehouse_id));

    order_line_index_scan_executor_->SetValues(order_line_key_values);

    auto order_line_index_scan_res =
        ExecuteReadTest(order_line_index_scan_executor_);
    ...
    new_order_delete_index_scan_executor_->ResetState();
    ...
    new_order_delete_index_scan_executor_->SetValues(
        new_order_delete_key_values);

    // Execute the query
    ExecuteDeleteTest(new_order_delete_executor_);
    ...
    orders_update_index_scan_executor_->ResetState();

    orders_update_index_scan_executor_->SetValues(orders_key_values);
    ...
    orders_target_list.emplace_back(
        COL_IDX_O_CARRIER_ID,
        expression::ExpressionUtil::ConstantValueFactory(orders_update_val));

    orders_update_executor_->SetTargetList(orders_target_list);

    // Execute the query
    ExecuteUpdateTest(orders_update_executor_);
    ...
    order_line_update_index_scan_executor_->ResetState();

    order_line_update_index_scan_executor_->SetValues(order_line_key_values);
    ...
    order_line_update_executor_->SetTargetList(order_line_target_list);

    ExecuteUpdateTest(order_line_update_executor_);
    ...
    auto plus_operator_expr = expression::ExpressionUtil::OperatorFactory(
        EXPRESSION_TYPE_OPERATOR_PLUS, VALUE_TYPE_INTEGER, tuple_val_expr,
        constant_val_expr);

    customer_target_list.emplace_back(COL_IDX_C_BALANCE, plus_operator_expr);

    customer_update_executor_->SetTargetList(customer_target_list);

    // Execute the query
    ExecuteUpdateTest(customer_update_executor_);
...
DeliveryPlans PrepareDeliveryPlan() {
  ...
  delivery_plans.new_order_delete_index_scan_executor_ =
      new_order_delete_index_scan_executor;
  ...
  delivery_plans.orders_update_index_scan_executor_ =
      orders_update_index_scan_executor;
  delivery_plans.orders_update_executor_ = orders_update_executor;

  delivery_plans.order_line_index_scan_executor_ =
      order_line_index_scan_executor;
  delivery_plans.order_line_update_index_scan_executor_ =
      order_line_update_index_scan_executor;
...
bool RunDelivery(DeliveryPlans &delivery_plans, const size_t &thread_id) {
  ...
  for (int d_id = 0; d_id < state.districts_per_warehouse; ++d_id) {
    ...
    delivery_plans.new_order_index_scan_executor_->SetValues(
        new_order_key_values);
    ...
    auto new_order_ids =
        ExecuteReadTest(delivery_plans.new_order_index_scan_executor_);
    ...
    auto orders_ids =
        ExecuteReadTest(delivery_plans.orders_index_scan_executor_);
    ...
    delivery_plans.order_line_index_scan_executor_->SetValues(
        order_line_key_values);

    auto order_line_index_scan_res =
        ExecuteReadTest(delivery_plans.order_line_index_scan_executor_);
    ...
    delivery_plans.new_order_delete_index_scan_executor_->SetValues(
        new_order_delete_key_values);
    ...
    delivery_plans.orders_update_index_scan_executor_->SetValues(
        orders_key_values);
    ...
    delivery_plans.order_line_update_index_scan_executor_->SetValues(
        order_line_key_values);
    ...
    delivery_plans.order_line_update_executor_->SetTargetList(
        order_line_target_list);
    ...
    delivery_plans.customer_index_scan_executor_->SetValues(
        customer_key_values);
    ...
    delivery_plans.customer_update_executor_->SetTargetList(
