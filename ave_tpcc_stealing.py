...
with open(filename) as inf:
    ...
    for i in range(0, count, 10):
        ...
        ### steal ###
        steal = [x.split()[9:10] for x in content[i:i+10]]
        elements_s = []
        for item in steal:
            elements_s.append(float(item[0]))
        ave_steal = sum(elements_s)/len(elements_s)
        ### steal rate ###
        steal_rate = [x.split()[10:11] for x in content[i:i+10]]
        elements_sr = []
        for item in steal_rate:
            elements_sr.append(float(item[0]))
        ave_steal_rate = sum(elements_sr)/len(elements_sr)

        result.write(str(ave_throughput) + ' ' + str(ave_abort) + ' '+ str(ave_delay) + ' '+str(ave_thread)+ ' '+ str(ave_steal) + ' ' + str(ave_steal_rate) + '\n')
