...
output_file = "./tpcc-cc-abort-test.summary"
############################################
...
for i in range(1, 12):
    cmd = "./src/tpcc -k 0.03 -b" + " " + str(i) + " " + "-p pcc -g co -d 5 -w 11 -q control -z 1"
    pid = subprocess.Popen(cmd, shell=True)
    pid.wait()

for i in range(1, 12):
    cmd = "./src/tpcc -k 0.03 -b" + " " + str(i) + " " + "-p to -g co -d 5 -w 11 -q control -z 1"
    pid = subprocess.Popen(cmd, shell=True)
    pid.wait()
