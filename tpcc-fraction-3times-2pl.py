...
########## literal sum single  ##########
for i in range(1, 12):
    cmd = "./src/tpcc -k 0.1 -b" + " " + str(i) + " " + "-p pcc -g co -d 5 -w 11 -n 1 -z hash -l -j -h"
    pid = subprocess.Popen(cmd, shell=True)
    pid.wait()
    for k in range(1, 4):
        cmd = "./src/tpcc -k 0.1 -b" + " " + str(i) + " " + "-p pcc -g co -d 5 -w 11 -n 1 -z hash -l -f -h"
        pid = subprocess.Popen(cmd, shell=True)
        pid.wait()
...
for i in range(2000, 20000, 2000):
    cmd = "./src/tpcc -b 11 -k 0.1 -p pcc -g co -d 5 -w 11 -n 1 -v" + " " + str(i) + " " + "-z hash -l -j -h"
    pid = subprocess.Popen(cmd, shell=True)
    pid.wait()
    for k in range(1, 4):
        cmd = "./src/tpcc -b 11 -k 0.1 -p pcc -g co -d 5 -w 11 -n 1 -v" + " " + str(i) + " " + "-z hash -l -f -h"
        pid = subprocess.Popen(cmd, shell=True)
        pid.wait()
